...
use kifmm::traits::types::NumberOfFlops;
...
fn count_flops_blas_m2l<T: RlstScalar<Real = T> + Epsilon + MatrixRsvd + Float + SampleUniform>(
    n_points: usize,
    n_vecs: usize,
    e: usize,
        ...
        for (n_points_key, n_points_val) in n_points_map {
            ...
            if let Some(digits_map) = n_points_val.get("digits").and_then(Value::as_mapping) {
                for (digit_key, params_val) in digits_map {
                    ...
                    let nflops;
                    if precision == "fp32" {
                        nflops = count_flops_fft_m2l::<f32>(
                        ...
                        nflops = count_flops_fft_m2l::<f64>(
                            n_points.try_into().unwrap(),
                            e.try_into().unwrap(),
                            Some(block_size.try_into().unwrap()),
                            Some(depth),
                    ...
                    println!("precision: {precision}, m2l: fft, n_points: {n_points}, digits: {digits} nflops: {nflops}");
                }
            }
        }
        ...
        for (n_points_key, n_points_val) in n_points_map {
            ...
            if let Some(digits_map) = n_points_val.get("digits").and_then(Value::as_mapping) {
                for (digit_key, params_val) in digits_map {
                    ...
                    let nflops;
                    if precision == "fp32" {
                        nflops = count_flops_blas_m2l::<f32>(
                            n_points.try_into().unwrap(),
                            1,
                            e.try_into().unwrap(),
                        ...
                        nflops = count_flops_blas_m2l::<f64>(
                            n_points.try_into().unwrap(),
                            1,
                            e.try_into().unwrap(),
                            Some(surface_diff.try_into().unwrap()),
                    ...
                    println!("precision: {precision}, m2l: blas, n_points: {n_points}, digits: {digits} nflops: {nflops}");
                }
            }
        }
    }
