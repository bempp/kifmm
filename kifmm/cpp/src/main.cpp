...
std::vector<float> generate_random_coordinates(size_t num_points,
                                               float min_range,
                                               float max_range) {
...
// Function to generate random 3D coordinate data
std::vector<float> generate_random_charges(size_t num_points, float min_range,
                                           float max_range) {
  // Random number generation
  std::random_device rd;  // Obtain a random number from hardware
  std::mt19937 gen(rd()); // Seed the generator
...
int main(void) {
  ...
  std::vector<float> coordinates =
      generate_random_coordinates(num_points, min_range, max_range);
  ...
  // LaplaceBlas32* fmm = laplace_blas_f32(sources_ptr, num_points,
  // targets_ptr, num_points, charges_ptr, num_points);

  KiFmm<float> fmm(coordinates, coordinates, charges);

  fmm.printCoordinates();
