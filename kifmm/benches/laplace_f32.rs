...
fn laplace_potentials_f32_1e6(c: &mut Criterion) {
    ...
    // // 3 Digits
    {
        ...
        // group.bench_function(format!("N=1e6, M2L=FFT digits=3"), |b| {
        //     b.iter(|| fmm_fft.evaluate())
        // });

        // group.bench_function(format!("N=1e6, M2L=FFT digits=3, M2L "), |b| {
        //     b.iter(|| {
        ...
        // group.bench_function(format!("N=1e6, M2L=FFT digits=3, P2P "), |b| {
        ...
        group.bench_function(format!("N=1e6, M2L=BLAS digits=3"), |b| {
            b.iter(|| fmm_blas.evaluate())
        });

        group.bench_function(format!("N=1e6, M2L=BLAS digits=3, M2L "), |b| {
            b.iter(|| {
        ...
        group.bench_function(format!("N=1e6, M2L=BLAS digits=3, P2P "), |b| {
    ...
    // 4 Digits
    {
        ...
        // group.bench_function(format!("N=1e6, M2L=FFT digits=4"), |b| {
        //     b.iter(|| fmm_fft.evaluate())
        // });

        // group.bench_function(format!("N=1e6, M2L=FFT digits=4, M2L "), |b| {
        //     b.iter(|| {
        //         for level in 2..=fmm_fft.tree().target_tree().depth() {
        //             fmm_fft.m2l(level).unwrap();
        //         }
        ...
        // group.bench_function(format!("N=1e6, M2L=FFT digits=4, P2P "), |b| {
        //     b.iter(|| fmm_fft.p2p().unwrap())
        // });
        ...
        group.bench_function(format!("N=1e6, M2L=BLAS digits=4"), |b| {
            b.iter(|| fmm_blas.evaluate())
        });

        group.bench_function(format!("N=1e6, M2L=BLAS digits=4, M2L "), |b| {
            b.iter(|| {
                for level in 2..=fmm_blas.tree().target_tree().depth() {
                    fmm_blas.m2l(level).unwrap();
                }
        ...
        group.bench_function(format!("N=1e6, M2L=BLAS digits=4, P2P "), |b| {
            b.iter(|| fmm_blas.p2p().unwrap())
        });
    }
}
...
fn laplace_potentials_f32_8e6(c: &mut Criterion) {
    ...
    // 3 Digits
    {
        ...
        // group.bench_function(format!("N=8e6, M2L=FFT digits=3"), |b| {
        //     b.iter(|| fmm_fft.evaluate())
        // });

        // group.bench_function(format!("N=8e6, M2L=FFT digits=3, M2L "), |b| {
        //     b.iter(|| {
        //         for level in 2..=fmm_fft.tree().target_tree().depth() {
        //             fmm_fft.m2l(level).unwrap();
        //         }
        ...
        // group.bench_function(format!("N=8e6, M2L=FFT digits=3, P2P "), |b| {
        //     b.iter(|| fmm_fft.p2p().unwrap())
        // });
        ...
        group.bench_function(format!("N=8e6, M2L=BLAS digits=3"), |b| {
            b.iter(|| fmm_blas.evaluate())
        });

        group.bench_function(format!("N=8e6, M2L=BLAS digits=3, M2L "), |b| {
            b.iter(|| {
                for level in 2..=fmm_blas.tree().target_tree().depth() {
                    fmm_blas.m2l(level).unwrap();
                }
        ...
        group.bench_function(format!("N=8e6, M2L=BLAS digits=3, P2P "), |b| {
            b.iter(|| fmm_blas.p2p().unwrap())
        });
    }
    ...
    // 4 Digits
    {
        ...
        // group.bench_function(format!("N=8e6, M2L=FFT digits=4"), |b| {
        //     b.iter(|| fmm_fft.evaluate())
        // });

        // group.bench_function(format!("N=8e6, M2L=FFT digits=4, M2L "), |b| {
        //     b.iter(|| {
        //         for level in 2..=fmm_fft.tree().target_tree().depth() {
        //             fmm_fft.m2l(level).unwrap();
        //         }
        ...
        // group.bench_function(format!("N=8e6, M2L=FFT digits=4, P2P "), |b| {
        //     b.iter(|| fmm_fft.p2p().unwrap())
        // });
        ...
        group.bench_function(format!("N=8e6, M2L=BLAS digits=4"), |b| {
            b.iter(|| fmm_blas.evaluate())
        });

        group.bench_function(format!("N=8e6, M2L=BLAS digits=4, M2L "), |b| {
            b.iter(|| {
                for level in 2..=fmm_blas.tree().target_tree().depth() {
                    fmm_blas.m2l(level).unwrap();
                }
        ...
        group.bench_function(format!("N=8e6, M2L=BLAS digits=4, P2P "), |b| {
            b.iter(|| fmm_blas.p2p().unwrap())
        });
    }
}
