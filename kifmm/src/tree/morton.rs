...
fn point_to_anchor<T: RlstScalarFloat<Real = T> + RlstScalar + Float + ToPrimitive>(
    point: &[T::Real; 3],
    level: u64,
    domain: &Domain<T>,
...
impl<T: RlstScalarFloat<Real = T> + Float> FmmTreeNode<T> for MortonKey<T> {
    fn convolution_grid(
        &self,
...
mod test {
    ...
    fn less_than<T: RlstScalarFloat<Real = T> + Float>(a: &MortonKey<T>, b: &MortonKey<T>) -> Option<bool> {
