...
use super::types::{FftError, Plan, Plan32, Plan64, ShapeInfo};
...
impl RealToComplexFft3D for f32 {
    ...
    fn r2c_batch_par(
        ...
        it_in_.zip(it_out).for_each(|(in_, out)| {
            let p = plan;
            unsafe { ffi::fftwf_execute_dft_r2c(p.plan.0, in_.as_mut_ptr(), out.as_mut_ptr()) };
        });

        unsafe {
            ffi::fftwf_destroy_plan(plan.plan.0);
        };
    ...
    fn c2r_batch_par(
        ...
        it_in_.zip(it_out).for_each(|(in_, out)| {
            let p = plan;
            unsafe { ffi::fftwf_execute_dft_c2r(p.plan.0, in_.as_mut_ptr(), out.as_mut_ptr()) }
...
impl RealToComplexFft3D for f64 {
    ...
    fn r2c_batch_par(
        ...
        it_in_.zip(it_out).for_each(|(in_, out)| {
            let p = plan;
            unsafe { ffi::fftw_execute_dft_r2c(p.plan.0, in_.as_mut_ptr(), out.as_mut_ptr()) };
        });

        unsafe {
            ffi::fftw_destroy_plan(plan.plan.0);
        };
    ...
    fn c2r_batch_par(
        ...
        it_in_.zip(it_out).for_each(|(in_, out)| {
            let p = plan;
            unsafe { ffi::fftw_execute_dft_c2r(p.plan.0, in_.as_mut_ptr(), out.as_mut_ptr()) }
