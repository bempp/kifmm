...
use mpi::{datatype::{Partition, PartitionMut}, topology::SimpleCommunicator, traits::{Communicator, CommunicatorCollectives, Equivalence}, Count};
...

{
    ...
    if k > arr.len() {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "k must be less than length of array"))
    } else if k == 0 {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "k must be greater than 0"))
    }
    ...
    let local_samples = sample_idxs.into_iter().map(|i|
        arr[i].clone()
    ).collect_vec();
    ...
    let counts_snd = splitter_indices.iter().map(|(l, r)| { if r-l > 0 {
        r-l + 1 } else { 0 }}).collect_vec();
    ...
    let mut partition_received = PartitionMut::new(&mut received[..], counts_recv, &displs_recv[..]);
