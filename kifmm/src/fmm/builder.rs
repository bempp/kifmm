...
impl SingleNodeBuilderLaplaceMetal
where
    Array<f32, BaseArray<f32, VectorContainer<f32>, 2>, 2>: MatrixSvd<Item = f32>,
    KiFmmLaplaceMetal: SourcetoTargetTranslationMetadata
        + SourceAndTargetTranslationMetadata
        + FmmMetadata<Scalar = f32>,
{
    ...
    pub fn tree(
        mut self,
        sources: &Coordinates<f32>,
        targets: &Coordinates<f32>,
        n_crit: Option<u64>,
        sparse: bool,
    ) -> Result<Self, std::io::Error> {
        let [nsources, dims] = sources.shape();
    ...
    pub fn parameters(
        ...
        charges: &Charges<f32>,
        expansion_order: usize,
        kernel: Laplace3dKernel<f32>,
        eval_type: EvalType,
        metal_level: u64,
        source_to_target: BlasFieldTranslationSaRcmpLaplaceMetal,
    ) -> Result<Self, std::io::Error> {
        if self.tree.is_none() {
            Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
            ...
            self.kernel_eval_type = Some(eval_type);
            self.source_to_target = Some(source_to_target);
            self.metal_level = Some(metal_level);
    ...
    /// Finalize and build the single node FMM
    pub fn build(self) -> Result<KiFmmLaplaceMetal, std::io::Error> {
        if self.tree.is_none() {
            Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "Must create a tree, and FMM metadata before building",
            ...
            let mut result = KiFmmLaplaceMetal {
                tree: self.tree.unwrap(),
                expansion_order: self.expansion_order.unwrap(),
                ncoeffs: self.ncoeffs.unwrap(),
                source_to_target: self.source_to_target.unwrap(),
                ...
                kernel,
                dim,
                metal_level: self.metal_level.unwrap(),
                ..Default::default()
            };
