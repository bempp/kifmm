...
use rlst::{external::metal::MetalDataBuffer, rlst_dynamic_array2, Array, BaseArray, RlstScalar, SliceContainer, VectorContainer};
...
pub struct KiFmmMetalLaplace
{
    ...
    /// The pseudo-inverse of the dense interaction matrix between the upward check and upward equivalent surfaces.
    /// Store in two parts to avoid propagating error from computing pseudo-inverse
    pub uc2e_inv_1: Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>, // index corresponds to level

    /// The pseudo-inverse of the dense interaction matrix between the upward check and upward equivalent surfaces.
    /// Store in two parts to avoid propagating error from computing pseudo-inverse
    pub uc2e_inv_2: Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>, // index corresponds to level

    /// The pseudo-inverse of the dense interaction matrix between the downward check and downward equivalent surfaces.
    /// Store in two parts to avoid propagating error from computing pseudo-inverse
    pub dc2e_inv_1: Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>, // index corresponds to level

    /// The pseudo-inverse of the dense interaction matrix between the downward check and downward equivalent surfaces.
    /// Store in two parts to avoid propagating error from computing pseudo-inverse
    pub dc2e_inv_2: Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>, // index corresponds to level

    /// Data and metadata for field translations
    pub source_to_target: BlasFieldTranslationSaRcmp<f32>,

    /// The multipole translation matrices, for a cluster of eight children and their parent. Stored in Morton order.
    pub source: Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>, // index corresponds to level

    /// The metadata required for source to source translation
    pub source_vec: Vec<Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>>, // index corresponds to level

    /// The local to local operator matrices, each index is associated with a child box (in sequential Morton order).
    pub target_vec: Vec<Vec<Array<f32, BaseArray<f32, MetalDataBuffer, 2>, 2>>>, // index corresponds to level
...
impl Default for KiFmmMetalLaplace
{
    fn default() -> Self {
        KiFmmMetalLaplace {
            tree: SingleNodeFmmTree::default(),
            source_to_target: BlasFieldTranslationSaRcmp::default(),
