...
use crate::{
    fmm::{
        ...
        helpers::chunk_size,
        types::{FmmEvalType, KiFmm, KiFmmLaplaceMetal},
    },
    traits::{
...
impl SourceTranslation for KiFmmLaplaceMetal
where
    Self: FmmOperatorData,
{
    fn p2m(&self) -> Result<(), FmmError> {
        ...
        match self.fmm_eval_type {
            FmmEvalType::Vector => {
                let mut check_potentials = rlst_dynamic_array2!(f32, [n_leaves * self.ncoeffs, 1]);
                    ...
                    .for_each(
                        |((check_potential, upward_check_surface), charge_index_pointer)| {
                            ...
                            if nsources > 0 {
                                ...
                                let mut coordinates_col_major =
                                    rlst_dynamic_array2!(f32, [nsources, self.dim]);
                                coordinates_col_major.fill_from(coordinates_row_major.view());
                ...
                check_potentials
                    ...
                    .for_each(|((check_potential, multipole_ptrs), scale)| {
                        ...
                        let tmp = if self.kernel.is_homogenous() {
                            let mut scaled_check_potential =
                                rlst_dynamic_array2!(f32, [self.ncoeffs, chunk_size]);
                            scaled_check_potential.fill_from(check_potential);
                            scaled_check_potential.scale_inplace(scale[0]);
            ...
            FmmEvalType::Matrix(nmatvecs) => {
                let mut check_potentials =
                    rlst_dynamic_array2!(f32, [n_leaves * self.ncoeffs * nmatvecs, 1]);
                ...
                check_potentials
                    ...
                    .for_each(
                        |((check_potential, upward_check_surface), charge_index_pointer)| {
                            let coordinates_row_major: &[f32] = &coordinates[charge_index_pointer.0
                                * self.dim
                                ..charge_index_pointer.1 * self.dim];
                            let nsources = coordinates_row_major.len() / self.dim;
                ...
                check_potentials
                    ...
                    .for_each(|((check_potential, multipole_ptrs), scale)| {
                        ...
                        let tmp = if self.kernel.is_homogenous() {
                            let mut scaled_check_potential =
                                rlst_dynamic_array2!(f32, [self.ncoeffs, nmatvecs]);
