    ...
    fn source(&mut self) {
        ...
        let iterator = if self.variable_expansion_order() {
            self.equivalent_surface_order
                ...
                .collect_vec()
                .into_iter()
                .zip(self.check_surface_order.iter().map(|&e| e as i32).collect_vec())
                .collect_vec()
        } else {
            vec![
                (
                    *self.equivalent_surface_order.last().unwrap() as i32,
                    *self.check_surface_order.last().unwrap() as i32
                );
                (depth + 1) as usize
            ]
        };

        // Partition iterator across MPI processes
        let size = self.communicator.size();
        let iterator_size = iterator.len() as i32;
        let q = iterator_size / size; // Base calculations per rank
        let r = iterator_size % size; // Extra calculations to distribute
        let distributed_load = (0..size).map(|i| if i < r { q + 1 } else { q }).collect_vec();
        use itertools::Itertools;

        let mut start = 0;
        let distributed_load_displacements = distributed_load.iter().map(|&load| {
            let end = start + load;
            let range = (start, end);
            start = end;
            range
        ...
        let (l, r) = distributed_load_displacements[self.communicator.rank() as usize];
        let local_iterator = &iterator[l as usize..r as usize];

        // Now compute local portion
        for &(equivalent_surface_order, check_surface_order) in local_iterator
        {
            let equivalent_surface_order = equivalent_surface_order as usize;
            let check_surface_order = check_surface_order as usize;
