use green_kernels::{
    helmholtz_3d::Helmholtz3dKernel, laplace_3d::Laplace3dKernel, traits::Kernel as KernelTrait, types::GreenKernelEvalType
};
...
use crate::{
    fmm::{
        ...
        helpers::{
            multi_node::{
                all_gather_v_serialised, coordinate_index_pointer_multi_node,
                deserialise_nested_array, deserialise_nested_vec, deserialise_vec,
                deserialise_vec_blas_metadata_sarcmp, deserialise_vec_fft_metadata,
                leaf_expansion_pointers_multi_node, leaf_surfaces_multi_node,
                level_expansion_pointers_multi_node, level_index_pointer_multi_node,
                potential_pointers_multi_node, serialise_nested_array, serialise_nested_vec,
                serialise_vec, serialise_vec_blas_metadata_sarcmp, serialise_vec_fft_metadata,
            },
            single_node::{flip3, homogenous_kernel_scale, ncoeffs_kifmm, optionally_time},
        },
        types::{BlasMetadataSaRcmp, FftMetadata, KiFmmMulti, NeighbourhoodCommunicator},
    },
    ...
    BlasFieldTranslationSaRcmp, FftFieldTranslation, FmmSvdMode, KiFmm, SingleNodeFmmTree,
};
    ...
    Self: DataAccessMulti + MetadataAccess,
{
    fn source(&mut self) {
        ...
        let iterator = if self.equivalent_surface_order.len() > 1 {
            ...
            let eq_vec = vec![eq_value; total_depth as usize];
            let ch_vec = vec![ch_value; total_depth as usize];

            (0..total_depth)
                .zip(eq_vec.into_iter())
        ...
        let uc2e_inv_1_r_serialised = serialise_nested_array(&uc2e_inv_1_r);
        let uc2e_inv_2_r_serialised = serialise_nested_array(&uc2e_inv_2_r);
        ...
        let (mut uc2e_inv_1, mut rest) = deserialise_nested_array(&uc2e_inv_1_serialised);
        while rest.len() > 0 {
        ...
        let (mut uc2e_inv_2, mut rest) = deserialise_nested_array(&uc2e_inv_2_serialised);
        while rest.len() > 0 {
            let (mut t1, t2) = deserialise_nested_array::<Scalar>(rest);
        ...
        let mut source_2 = Vec::new();
        let mut source_vec_2 = Vec::new();
        ...
        let iterator = if self.equivalent_surface_order.len() > 1 {
            ...
            (0..total_depth)
                .zip(
                    vec![*self.check_surface_order.last().unwrap(); total_depth as usize]
                        .into_iter()
                        .zip(vec![
        ...
        for (level, (check_surface_order_parent, equivalent_surface_order_child)) in iterator {
            ...
            let mut m2m_vec_1 = Vec::new();

            let mut m2m_2 =
                rlst_dynamic_array2!(Scalar, [n_equiv_surface_parent, 8 * n_equiv_surface_child]);
            let mut m2m_vec_2 = Vec::new();
            ...
            for (i, child) in children.iter().enumerate() {
                ...
                m2m_1.data_mut()[l..r].copy_from_slice(tmp.data());
                m2m_vec_1.push(tmp);

                m2m_2.data_mut()[l..r].copy_from_slice(tmp2.data());
                m2m_vec_2.push(tmp2);
            }

            source.push(m2m_1);
            source_vec.push(m2m_vec_1);

            source_2.push(m2m_2);
            source_vec_2.push(m2m_vec_2);
        }

        self.global_fmm.source = source_2;
        self.global_fmm.source_vec = source_vec_2;
        self.source = source;
