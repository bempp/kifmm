        ...
        for &equivalent_surface_order in iterator_r.iter() {
            ...
            for freq in 0..transform_size {
                ...
                for kernel_f in kernel_data_f.iter().take(NHALO) {
                    ...
                    let mut k_ft = DynArray::<<Scalar as DftType>::OutputType, _>::from_shape([
                        NSIBLINGS, NSIBLINGS,
                    ]);
...
where
    Scalar: RlstScalar + Default + MatrixRsvd + Equivalence + Lapack + Gemm + Float,
        ...
        for &(equivalent_surface_order, check_surface_order) in iterator_r.iter() {
            ...
            match &self.source_to_target.svd_mode {
                ...
                FmmSvdMode::Deterministic => {
                    let (s_tmp, u_tmp, vt_tmp) = se2tc_fat.svd(SvdMode::Compact).unwrap();
                    u_big.fill_from(&u_tmp);
                    vt_big.fill_from(&vt_tmp);
                    sigma.copy_from_slice(s_tmp.data().unwrap());
                }
            }
            ...
            if self.source_to_target.surface_diff() == 0 {
                ...
                match &self.source_to_target.svd_mode {
                    ...
                    FmmSvdMode::Deterministic => {
                        ...
                        let (_, _, st_tmp) = se2tc_thin.svd(SvdMode::Compact).unwrap();
                        st = st_tmp;
                    }
                }
            }
            ...
            (0..NTRANSFER_VECTORS_KIFMM).into_par_iter().for_each(|i| {
                ...
                let tmp = empty_array::<Scalar, 2>()
                    .simple_mult_into_resize(sigma_mat.r(), rlst::dot!(vt_block.r(), s_trunc.r()));
