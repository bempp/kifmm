...
use rlst::{
    ...
    DynArray, Gemm, Lapack, MultIntoResize, RlstScalar, SliceArray,
};
use rlst::{EvaluateObject, SingularValueDecomposition};
...
use crate::{
    fmm::{
        ...
        types::{
            ...
            BlasMetadataAca, BlasMetadataIa, BlasMetadataSaRcmp, FftFieldTranslation, FftMetadata,
            FmmSvdMode,
        },
    },
...
impl<Scalar> SourceToTargetTranslationMetadata
    for KiFmm<Scalar, Helmholtz3dKernel<Scalar>, BlasFieldTranslationAca<Scalar>>
where
    ...
    <<Scalar as Upcast>::Higher as RlstScalar>::Real: Epsilon + Cast<Scalar::Real>,
{
    ...
    fn source_to_target(&mut self) {
        ...
        let mut result = BlasFieldTranslationAca::<Scalar>::default();
        ...
        for ((level, equivalent_surface_order), check_surface_order) in iterator {
            ...
            let mut level_result = BlasMetadataAca::default();
            let mut level_u = Vec::new();
            let mut level_vt = Vec::new();

            transfer_vectors.into_iter().for_each(|t| {
...
where
    Scalar: RlstScalar<Complex = Scalar> + Default + MatrixRsvd + Lapack + Gemm,
    <Scalar as RlstScalar>::Real: Default,
{
    fn displacements(&mut self, start_level: Option<u64>) {
        let mut displacements = Vec::new();
                ...
                .for_each(|(i, t)| {
                    ...
                    match &self.source_to_target.svd_mode {
                        ...
                        FmmSvdMode::Deterministic => {
                            (sigma, u, vt) = {
                                let (sigma, u, vt) = tmp_gram.svd(SvdMode::Compact).unwrap();
                                (sigma.data().unwrap().to_vec(), u, vt)
                            }
                        }
                    }
...
where
    Scalar: RlstScalar<Complex = Scalar> + Default + MatrixRsvd + Gemm + Lapack,
    <Scalar as RlstScalar>::Real: Default,
{
    fn displacements(&mut self, start_level: Option<u64>) {
        let mut displacements = Vec::new();
    ...
    fn source_to_target(&mut self) {
        ...
        for ((level, equivalent_surface_order), check_surface_order) in iterator {
            ...
            match &self.source_to_target.svd_mode {
                &FmmSvdMode::Random {
                    ...
                    let mut se2tc_fat_transpose =
                        DynArray::<Scalar, _>::from_shape(se2tc_fat.r().transpose().shape());
                    se2tc_fat_transpose
                        .r_mut()
                ...
                FmmSvdMode::Deterministic => {
                    (sigma, u_big, vt_big) = {
                        let (sigma, u_big, vt_big) = se2tc_fat.svd(SvdMode::Compact).unwrap();
                        (sigma.data().unwrap().to_vec(), u_big, vt_big)
                    }
                    // se2tc_fat
                    //     .into_svd_alloc(
                    //         u_big.r_mut(),
            ...
            if self.source_to_target.surface_diff() == 0 {
                st = DynArray::<Scalar, _>::from_shape(u_big.r().transpose().shape());
                ...
                match &self.source_to_target.svd_mode {
                    ...
                    FmmSvdMode::Deterministic => {
                        (_, _, st) = se2tc_thin.svd(SvdMode::Compact).unwrap();
                    }
                }
            }
            ...
            (0..NTRANSFER_VECTORS_KIFMM).into_par_iter().for_each(|i| {
                let vt_block = vt
                    .r()
                    .into_subview([0, i * n_cols], [cutoff_rank, n_cols])
                    .eval();
                ...
                let (sigma_i, u_i, vt_i) = tmp.svd(SvdMode::Full).unwrap();

                let sigma_i = sigma_i.data().unwrap().to_vec();
