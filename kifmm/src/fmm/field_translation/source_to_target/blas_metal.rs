    ...
    Self: FmmOperatorData,
{
    ...
    fn displacements(&self, level: u64) -> Vec<Mutex<Vec<i64>>> {
        ...
        let tree = &self.tree;
        let all_transfer_vectors = &self.source_to_target.transfer_vectors;
        let level_index_pointer_locals = &self.level_index_pointer_locals;
            ...
            .for_each(|(source_idx, source)| {
                ...
                let interaction_list = source
                    ...
                    .filter(|pnc| {
                        !source.is_adjacent(pnc)
                            && tree.target_tree().all_keys_set().unwrap().contains(pnc)
                    })
                    .collect_vec();
                ...
                for (tv_idx, tv) in all_transfer_vectors.iter().enumerate() {
                    ...
                    if transfer_vectors_set.contains(&tv.hash) {
                        ...
                        let &target_idx = level_index_pointer_locals[level as usize]
                            .get(target)
                            .unwrap();
        ...
        match self.fmm_eval_type {
            FmmEvalType::Vector => {
                ...
                let mut compressed_multipoles;
                {
                    ...
                    if self.kernel.is_homogenous() {
                        compressed_multipoles.scale_inplace(
                            homogenous_kernel_scale::<f32>(level)
                                * m2l_scale::<f32>(level).unwrap(),
                        );
                    }
                }

                let [nrows, ncols] = compressed_multipoles.shape();

                let mut compressed_multipoles_t = rlst_dynamic_array2!(f32, [ncols, nrows]);
                compressed_multipoles_t.view_mut().fill_from(compressed_multipoles.view().transpose());
...

                {
                    ...
                    let cutoff_rank = self.source_to_target.cutoff_rank;
                    ...
                    if level < 6 {
                        ...
                        (0..NTRANSFER_VECTORS_KIFMM)
                            ...
                            .for_each(|((c_idx, multipole_idxs), local_idxs)| {
                                ...
                                let s = Instant::now();
                                let c_u_sub_metal = &self.source_to_target.metadata
                                    [m2l_operator_index]
                                    .c_u_metal[c_idx];
                                ...
                                for (i, &multipole_idx) in multipole_idxs.iter().enumerate() {
                                    ...
                                    compressed_multipoles_subset_metal.data_mut()
                                        ...
                                        .copy_from_slice(
                                            &compressed_multipoles_t.data()[multipole_idx
                                                * cutoff_rank
                                                ..(multipole_idx + 1) * cutoff_rank],
                                        );
                                }

                                let [nrows, _] = c_vt_sub_metal.shape();
                                let [ncols, _] = compressed_multipoles_subset_metal.shape();
                                let mut tmp = rlst_metal_array2!(&device, f32, [nrows, ncols]);
                                ...
                                let [nrows, _] = c_u_sub_metal.shape();
                                let [_, ncols] = tmp.shape();
                                let mut compressed_check_potential =
                                    rlst_metal_array2!(&device, f32, [nrows, ncols]);

                                compressed_check_potential.view_mut()
                                    .metal_mult_into(
                                    rlst::TransMode::NoTrans,
                                    rlst::TransMode::NoTrans,
                                    1.0,
                    ...
                    (0..NTRANSFER_VECTORS_KIFMM)
                        ...
                        .for_each(|((c_idx, multipole_idxs), local_idxs)| {
                            ...
                            // );
                                ...
                                let device = MetalDevice::from_default();
                                let mut compressed_multipoles_subset_metal_t = rlst_metal_array2!(
                                    &device,
                                    f32,
                                    [multipole_idxs.len() * nmatvecs, self.source_to_target.cutoff_rank]
                                );
                                ...
                                multipole_idxs.iter().enumerate()
                            {
                                ...
                                for charge_vec_idx in 0..nmatvecs {
                                    let charge_vec_displacement =
                                        ...
                                        compressed_multipoles_subset_metal_t.data_mut()[key_displacement_local
                                        + charge_vec_displacement
                                        ..key_displacement_local
                                            + charge_vec_displacement
                                            + self.source_to_target.cutoff_rank]
                            ...
                            let [a, b] = c_vt_sub_metal.shape();
                            let [c, d] = c_u_sub_metal.shape();
                            let mut left = rlst_metal_array2!(&device, f32, [a, d]);
                            left.view_mut().metal_mult_into(
                                rlst::TransMode::NoTrans,
                                rlst::TransMode::NoTrans,
                                1.0,
                                c_vt_sub_metal.view(),
                            ...
                            let [ncols, _] = compressed_multipoles_subset_metal_t.shape();
                            let [nrows, _] = left.shape();
                            ...
                            compressed_check_potential.view_mut().metal_mult_into(
                                rlst::TransMode::NoTrans,
                                rlst::TransMode::Trans,
                                1.0,
                                left.view(),
                                compressed_multipoles_subset_metal_t.view(),
                                0.0,
                            );
