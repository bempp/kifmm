...
use crate::{
    fmm::{
        ...
        types::{BlasFieldTranslationSa, FmmEvalType, SendPtrMut},
        KiFmm,
    },
    traits::{
...
impl<Scalar, Kernel> KiFmm<Scalar, Kernel, BlasFieldTranslationSa<Scalar>>
where
    Scalar: RlstScalar + Default,
    Kernel: KernelTrait<T = Scalar> + FmmKernel + Default,
    <Scalar as RlstScalar>::Real: Default,
    ...
    fn displacements(&self, level: u64) -> Vec<Mutex<Vec<i64>>> {
        ...
        sources
            ...
            .for_each(|(source_idx, source)| {
                ...
                // Mark items in interaction list for scattering
                for (tv_idx, tv) in self.source_to_target.transfer_vectors[level as usize].iter().enumerate() {
                    let mut all_displacements_lock = all_displacements[tv_idx].lock().unwrap();
                    if transfer_vectors_set.contains(&tv.hash) {
                        // Look up scatter location in target tree
                        let target =
...
impl<Scalar, Kernel> SourceToTargetTranslation
    for KiFmm<Scalar, Kernel, BlasFieldTranslationSa<Scalar>>
where
    Scalar: RlstScalar + Default,
    Kernel: KernelTrait<T = Scalar> + FmmKernel + Default + Send + Sync,
    <Scalar as RlstScalar>::Real: Default,
    ...
    fn m2l(&self, level: u64) {
        ...
        match self.fmm_eval_type {
            FmmEvalType::Vector => {
                ...
                let check_potentials = rlst_dynamic_array2!(
                    Scalar,
                    [
                        self.ncoeffs,
                        ntargets
                ...
                let level_check_potentials = check_potentials_ptrs
                    .iter()
                    .map(Mutex::new)
                    .collect_vec();
                ...
                // 1. Apply BLAS operation
                {
                    (0..NTRANSFER_VECTORS_KIFMM)
                        ...
                        .for_each(|((c_idx, multipole_idxs), local_idxs)| {
                            let u =
                                &self.source_to_target.metadata[m2l_operator_index].u[c_idx];
                            let vt =
                                &self.source_to_target.metadata[m2l_operator_index].vt[c_idx];
                            ...
                            for (i, &multipole_idx) in multipole_idxs.iter().enumerate() {
                                multipoles_subset.data_mut()[i * self
                                    .source_to_target
                                    .cutoff_rank[m2l_operator_index]
                                    ..(i + 1)
                                        * self.source_to_target.cutoff_rank[m2l_operator_index]]
                                    .copy_from_slice(
                                        &self.multipoles[multipole_idx
                                            * self.ncoeffs
