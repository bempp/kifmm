...
pub fn compute_transfer_vectors_at_level<T>(level: u64) -> Vec<TransferVector<T>>
where
    T: RlstScalar + Float,
{
    let half = T::from(0.5).unwrap();
    ...
    // Encode point in centre of domain
    let key = MortonKey::<T>::from_point(&point, &domain, level);
...
mod test {
    ...
    fn test_compute_transfer_vectors_at_level() {
        let tvs = compute_transfer_vectors_at_level::<f64>(2);
        println!("len {:?}", tvs.len());
        assert!(tvs.len() == 189);

        let tvs = compute_transfer_vectors_at_level::<f64>(3);
        println!("len {:?}", tvs.len());
        assert!(tvs.len() == 316);

        let tvs = compute_transfer_vectors_at_level::<f64>(4);
        println!("len {:?}", tvs.len());
        assert!(tvs.len() == 316)
    }
}
