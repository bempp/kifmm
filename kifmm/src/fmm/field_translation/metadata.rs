    ...
    Self: SourceToTargetTranslation,
{
    fn source(&mut self) {
        ...
        for level in 0..self.expansion_order.len() {

            let expansion_order = self.expansion_order[level];
            // Compute required surfaces
            let upward_equivalent_surface = root.surface_grid(expansion_order, &domain, alpha_inner);
            ...
            let uc2e_inv_1_level = empty_array::<Scalar, 2>().simple_mult_into_resize(v.view(), mat_s.view());
            let uc2e_inv_2_level = ut;


            for (i, child) in children.iter().enumerate() {
                let child_upward_equivalent_surface =
                    child.surface_grid(expansion_order, &domain, alpha_inner);
                ...
                let tmp = empty_array::<Scalar, 2>().simple_mult_into_resize(
                    uc2e_inv_1_level.view(),
                    empty_array::<Scalar, 2>()
                        .simple_mult_into_resize(uc2e_inv_2_level.view(), pc2ce.view()),
                );
            ...
            uc2e_inv_1.push(uc2e_inv_1_level);
            uc2e_inv_2.push(uc2e_inv_2_level);
            m2m.push(m2m_level);
            m2m_vec.push(m2m_vec_level);
        }

        println!("HERE {:?} {:?} {:?}", m2m, uc2e_inv_1, uc2e_inv_2);
    ...
    fn target(&mut self) {
        ...
        // Compute required surfaces
        let downward_equivalent_surface = root.surface_grid(expansion_order, &domain, alpha_outer);
    ...
    Self: SourceToTargetTranslation,
{
    fn source(&mut self) {
        ...
        if self.expansion_order.len() > 1 {
            iterator = (0..=depth).zip(self.expansion_order.clone()).collect_vec();
        } else {
            iterator = (0..=depth).zip(vec![self.expansion_order[0]; (depth as usize)+1]).collect_vec();
        }
        ...
        for &(_level, expansion_order) in &iterator {
    ...
    fn target(&mut self) {
        ...
        for level in 0..depth {
            ...
            let downward_equivalent_surface =
                curr.surface_grid(expansion_order, &domain, alpha_outer);
            let downard_check_surface = curr.surface_grid(expansion_order, &domain, alpha_inner);
    ...
    <Scalar as RlstScalar>::Real: Default,
{
    ...
    fn metadata(&mut self, eval_type: EvalType, charges: &[Self::Scalar]) {
        ...
        let multipoles = vec![Scalar::default(); ncoeffs * nsource_keys * nmatvecs];
        let locals = vec![Scalar::default(); ncoeffs * ntarget_keys * nmatvecs];
        ...
        let source_leaf_scales =
            leaf_scales::<Scalar>(&self.tree.source_tree, self.kernel.is_homogenous(), ncoeffs);
        ...
        let level_multipoles =
            level_expansion_pointers(&self.tree.source_tree, ncoeffs, nmatvecs, &multipoles);

        let level_locals =
            level_expansion_pointers(&self.tree.source_tree, ncoeffs, nmatvecs, &locals);
...
mod test {
    ...
    fn test_blas_field_translation_helmholtz() {
        ...
        let mut multipole = rlst_dynamic_array2!(c64, [ncoeffs, 1]);
        for i in 0..ncoeffs {
    ...
    #[test]
    fn test_variable_expansion_order() {
        // Setup random sources and targets
        let nsources = 10000;
        let ntargets = 10000;
        let sources = points_fixture::<f64>(nsources, None, None, Some(1));
