...
use crate::{
    fmm::{
        ...
        helpers::chunk_size,
        types::{FmmEvalType, KiFmmLaplaceMetal},
        KiFmm,
    },
    traits::{
...
impl TargetTranslation for KiFmmLaplaceMetal
where
    Self: FmmOperatorData,
{
    fn l2l(&self, level: u64) -> Result<(), FmmError> {
        ...
        match self.fmm_eval_type {
            ...
            FmmEvalType::Matrix(nmatvecs) => {
                ...
                parent_locals
                    ...
                    .zip(child_locals.par_chunks_exact(NSIBLINGS))
                    .for_each(|(parent_local_pointers, child_locals_pointers)| {
                        let mut parent_locals = rlst_dynamic_array2!(f32, [self.ncoeffs, nmatvecs]);
                    ...
                    .for_each(
                        |(
                            ...
                            if ntargets > 0 {
                                ...
                                let mut target_coordinates_col_major =
                                    rlst_dynamic_array2!(f32, [ntargets, self.dim]);
                                target_coordinates_col_major
                                    .fill_from(target_coordinates_row_major.view());
