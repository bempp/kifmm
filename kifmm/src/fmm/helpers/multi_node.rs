...
// /// Form queries for admissable and inadmissable interactions contained on foreign MPI ranks.
// pub fn form_morton_queries<T>(
//     tree: &MultiNodeTree<T::Real, SimpleCommunicator>,
//     layout: &Layout<T::Real>,
//     admissible: bool,
// ) -> Query
// where
//     T: RlstScalar + Equivalence + Float,
...
//     if admissible {
//         // V list queries
//         for key in tree.all_keys().unwrap() {
...
//                     if let Some(&rank) = layout.rank_from_key(&key) {
...
//         }
//     } else {
//         for leaf in tree.all_leaves().unwrap() {
...
//     let mut send_counts = vec![0 as Count; tree.comm.size() as usize];
//     let mut send_marker = vec![0 as Rank; tree.comm.size() as usize];

//     for query in queries.iter() {
//         let rank = *layout.rank_from_key(query).unwrap();
//         ranks.push(rank);
