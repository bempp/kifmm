...
fn grid_search_laplace_fft<T>(
    ...
    expansion_order_vec: &Vec<usize>,
    depth_vec: &Vec<u64>,
    block_size_vec: &Vec<Option<usize>>,
) where
    Array<T, BaseArray<T, VectorContainer<T>, 2>, 2>: MatrixSvd<Item = T>,
    T: RlstScalar<Real = T>
        + AsComplex
    ...
    for (i, fmm) in fmms.lock().unwrap().iter() {
        ...
        writer
            .write_record(&[
                depth.to_string(),
                expansion_order.to_string(),
                block_size.unwrap_or(128).to_string(),
                time.to_string(),
                min_rel_err.to_string(),
                mean_rel_err.to_string(),
                max_rel_err.to_string(),
...
fn main() {
    ...
    let block_size_vec: Vec<Option<usize>> = vec![
        None,
        Some(8),
        Some(16),
        Some(64),
   ...
   //     "grid_search_laplace_fft_f64_m1_1".to_string(),
   //     &expansion_order_vec,
   //     &depth_vec,
   //     &block_size_vec
   // );
     ...
     grid_search_laplace_blas::<f64>(
         "grid_search_laplace_blas_f64_m1_7".to_string(),
         &expansion_order_vec,
         &svd_threshold_vec,
         &surface_diff_vec,
         &depth_vec,
