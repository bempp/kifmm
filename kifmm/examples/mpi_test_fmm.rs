...
fn main() {
    ...
    // Test FFT field translation
    {
        ...
        println!("RANK {:?} N local sources {:?}", multi_fmm.tree.source_tree.communicator.rank(), multi_fmm.tree.source_tree.n_coordinates_tot());
    ...
    //     let points = points_fixture::<f32>(n_points, None, None, Some(world.rank() as u64));
    //     let mut charges = rlst_dynamic_array1!(f32, [n_points]);
    //     charges.fill_from_seed_equally_distributed(0);
