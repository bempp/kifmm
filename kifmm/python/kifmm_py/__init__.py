...
class SortKind(Enum):
    """
    Parallel Sort strategy for distributed trees
    """
...
class MultiNodeTree:
    def __init__(
        ...
        """Constructor for MultiNodeTree

        Args:
            communicator (MPI_Comm): MPI Communicator for the distributed tree.
            ...
            local_depth (int): Must set local_depth, specifies the depth of shared memory trees.
            global_depth (int): Must set global_depth, specifies the total number of shared memory trees.
            sort_kind (SortKind): Must specify a parallel sort routine, SimpleSort (Bucket Sort), SampleSort, and Hyksort. For SampleSort 'n_samples' must be set
            prune_empty (bool, optional): Whether or not to discard empty leaf boxes, and their siblings and ancestors. Defaults to False.
            n_samples (int, optional): Must specify the number of samples per processor if using SampleSort
        """
        dim = 3
        try:
        ...
        try:
            valid = local_depth > 0 and global_depth > 0
            assert valid

        except:
            raise TypeError("local_depth and global_depth must be set, and be >=1")
        ...
        if self.sort_kind == SortKind.SampleSort:
            try:
                assert n_samples > 0
            except:
                ValueError("If 'SampleSort' is chosen, 'n_samples' must be set >= 1")
...
class KiFmmMulti:
    """Runtime distributed FMM object"""
    ...
    def __init__(
        ...
        """Constructor for distributed FMM

        Args:
            expansion_order (list[int], int): The expansion order of the FMM, if specifying a depth expansion order must be specified for each tree level in a list
            tree (obj): Currently only 'MultiNodeTree' supported
            field_translation (obj): Either 'FftFieldTranslation' or 'BlasFieldTranslation'.
            timed (bool): Optionally return operator runtimes.
        """
        ...
        if self._n_evals != 1:
            raise TypeError(
                "Currently only support vector lhs input for distributed FMM"
            )
        ...
        if isinstance(self._field_translation, FftFieldTranslation):
            if isinstance(self._field_translation.kernel, LaplaceKernel):
                if self._field_translation.kernel.dtype == np.float32:
                    ...
                    self._fmm = lib.laplace_fft_f64_mpi_alloc(
                        self._timed,
                        self._expansion_order_c,
                        self._n_expansion_order,
                        self._field_translation.kernel.eval_type,
                ...
                if self._field_translation.kernel.dtype == np.float32:
                    self._fmm = lib.helmholtz_fft_f32_mpi_alloc(
                        self._timed,
                        self._expansion_order_c,
                        self._n_expansion_order,
                        self._field_translation.kernel.eval_type,
                    ...
                    self._fmm = lib.helmholtz_fft_f64_mpi_alloc(
                        self._timed,
                        self._expansion_order_c,
                        self._n_expansion_order,
                        self._field_translation.kernel.eval_type,
            ...
            if isinstance(self._field_translation.kernel, LaplaceKernel):
                ...
                if self._field_translation.random:
                    if self._field_translation.kernel.dtype == np.float32:
                        self._fmm = lib.laplace_blas_rsvd_f32_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                        ...
                        self._fmm = lib.laplace_blas_rsvd_f64_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                    ...
                    if self._field_translation.kernel.dtype == np.float32:
                        self._fmm = lib.laplace_blas_svd_f32_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                        ...
                        self._fmm = lib.laplace_blas_svd_f64_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                ...
                if self._field_translation.random:
                    if self._field_translation.kernel.dtype == np.float32:
                        self._fmm = lib.helmholtz_blas_rsvd_f32_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                        ...
                        self._fmm = lib.helmholtz_blas_rsvd_f64_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                    ...
                    if self._field_translation.kernel.dtype == np.float32:
                        self._fmm = lib.helmholtz_blas_svd_f32_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
                        ...
                        self._fmm = lib.helmholtz_blas_svd_f64_mpi_alloc(
                            self._timed,
                            self._expansion_order_c,
                            self._n_expansion_order,
                            self._field_translation.kernel.eval_type,
    ...
    def _target_global_indices(self):
        global_indices_p = lib.global_indices_target_tree_mpi(self._fmm)
        ptr = ffi.cast("uintptr_t*", global_indices_p.data)
        tmp = np.frombuffer(
            ffi.buffer(ptr, global_indices_p.len * ffi.sizeof("uintptr_t")),
            dtype=np.uint64,
    ...
    def _source_tree_global_depth(self):
        self.source_tree_global_depth = lib.source_tree_global_depth_mpi(self._fmm)

    def _target_tree_local_depth(self):
        self.target_tree_local_depth = lib.target_tree_local_depth_mpi(self._fmm)

    def _target_tree_global_depth(self):
        self.target_tree_global_depth = lib.target_tree_global_depth_mpi(self._fmm)

    def _leaves_target_tree(self):
        morton_keys_p = lib.leaves_target_tree_mpi(self._fmm)
        ptr = ffi.cast("uint64_t*", morton_keys_p.data)
        self.leaves_target_tree = np.frombuffer(
            ffi.buffer(ptr, morton_keys_p.len * ffi.sizeof("uint64_t")), dtype=np.uint64
        )
    ...
    def _leaves_source_tree(self):
        morton_keys_p = lib.leaves_source_tree_mpi(self._fmm)
        ptr = ffi.cast("uint64_t*", morton_keys_p.data)
        self.leaves_source_tree = np.frombuffer(
            ffi.buffer(ptr, morton_keys_p.len * ffi.sizeof("uint64_t")), dtype=np.uint64
        )
    ...
    def _keys_target_tree(self):
        morton_keys_p = lib.keys_target_tree_mpi(self._fmm)
        ptr = ffi.cast("uint64_t*", morton_keys_p.data)
        self.keys_target_tree = np.frombuffer(
            ffi.buffer(ptr, morton_keys_p.len * ffi.sizeof("uint64_t")), dtype=np.uint64
        )
    ...
    def _keys_source_tree(self):
        morton_keys_p = lib.keys_source_tree_mpi(self._fmm)
        ptr = ffi.cast("uint64_t*", morton_keys_p.data)
        self.keys_source_tree = np.frombuffer(
            ffi.buffer(ptr, morton_keys_p.len * ffi.sizeof("uint64_t")), dtype=np.uint64
        )
    ...
    def _all_potentials(self):
        dim = 3
        if self._evaluated:
            self._all_potentials_p = lib.all_potentials_mpi(self._fmm)
    ...
    def surface(self, alpha, expansion_order, key):
        ...
        coords_p = lib.surface_mpi(alpha, expansion_order, key)
    ...
    def local(self, key):
        ...
        if self._evaluated:
            ...
            local = lib.local_mpi(self._fmm, key)
            local_p = local.data
            local_len = local.len
            tmp = KiFmm._cast_to_numpy_array(
                local_p, local_len, self.potential_dtype, ffi
    ...
    def multipole(self, key):
        ...
        if self._evaluated:
            ...
            multipole = lib.multipole_mpi(self._fmm, key)
            multipole_p = multipole.data
            multipole_len = multipole.len
            tmp = KiFmm._cast_to_numpy_array(
                multipole_p, multipole_len, self.potential_dtype, ffi
    ...
    def leaf_potentials(self, leaf):
        ...
        if self._evaluated:
            ...
            potentials = lib.leaf_potentials_mpi(self._fmm, leaf)
            n = potentials.n
            for i in range(0, n):
                potential_p = potentials.data[i].data
                potential_len = potentials.data[i].len
    ...
    def attach_charges_unordered(self, charges):
        ...
        """
        self._tree.new_charges(charges)
        lib.attach_charges_unordered_mpi(
            self._fmm, self._tree.charges_c, self._tree.n_charges
        )
    ...
    def attach_charges_ordered(self, charges):
        ...
        """
        self._tree.new_charges(charges)
        lib.attach_charges_ordered_mpi(
            self._fmm, self._tree.charges_c, self._tree.n_charges
        )
    ...
    def coordinates_source_tree(self, leaf):
        ...
        coords = None
        if self._evaluated:
            coords_p = lib.coordinates_source_tree_mpi(self._fmm, leaf)
            coords = KiFmm._cast_to_numpy_array(
                coord_p.data, coords_p.len, self._field_translation.kernel.dtype, ffi
            )
    ...
    def coordinates_target_tree(self, leaf):
        ...
        coords = None
        if self._evaluated:
            coords_p = lib.coordinates_target_tree_mpi(self._fmm, leaf)
            coords = KiFmm._cast_to_numpy_array(
                coords_p.data, coords_p.len, self._field_translation.kernel.dtype, ffi
            )
    ...
    def __del__(self):
        if self._evaluated:
            lib.free_fmm_evaluator_mpi(self._fmm)
            for ref in self._morton_keys_refs:
                lib.free_morton_keys(ref)
